...
package org.eclipse.osgi.technology.opentelemetry.core.sender.http.impl;
 ...
 * <a href="https://opentelemetry.io/docs/specs/otel/protocol/exporter/">OTLP
 * protocol</a> over HTTP/protobuf. The default endpoint is
 * {@code http://localhost:4318}. Signal-specific paths ({@code /v1/traces},
 * {@code /v1/metrics}, {@code /v1/logs}) are appended automatically.
 */
@ObjectClassDefinition(name = "OpenTelemetry OTLP/HTTP Exporter", description = "Exports telemetry (traces, metrics, logs) via OTLP/HTTP to an OpenTelemetry "
...
public @interface OtlpHttpOpenTelemetryConfiguration {
	...
	String PID = org.eclipse.osgi.technology.opentelemetry.core.sender.http.api.Constants.PID;
	...
	@AttributeDefinition(name = "Export Timeout (ms)", description = "Maximum time in milliseconds to wait for each export request to complete. "
			+ "If exceeded, the export is cancelled and data may be lost. Default: 10000")
	long timeout() default 10000;

	@AttributeDefinition(name = "Connection Timeout (ms)", description = "Maximum time in milliseconds to wait when establishing a connection to the "
			+ "OTLP endpoint. Default: 10000")
	long connectTimeout() default 10000;

	@AttributeDefinition(name = "Compression", description = "Compression algorithm applied to the export payload before sending. "
			+ "'gzip' reduces bandwidth usage at the cost of additional CPU. "
			+ "'none' sends payloads uncompressed.", options = { @Option(label = "None", value = "none"),
					@Option(label = "Gzip", value = "gzip") })
	String compression() default "none";

	@AttributeDefinition(name = "Headers", description = "Additional HTTP headers sent with every export request, specified as "
	...
	@AttributeDefinition(name = "Span Schedule Delay (ms)", description = "Delay between consecutive span exports in milliseconds. "
			+ "Lower values send data faster but increase network traffic. Default: 5000")
	long spanScheduleDelay() default 5000;

	@AttributeDefinition(name = "Span Max Export Batch Size", description = "Maximum number of spans exported per batch. Default: 512")
	int spanMaxExportBatchSize() default 512;

	@AttributeDefinition(name = "Span Max Queue Size", description = "Maximum number of spans queued before dropping. Default: 2048")
	int spanMaxQueueSize() default 2048;

	@AttributeDefinition(name = "Metric Export Interval (ms)", description = "Interval between consecutive metric exports in milliseconds. "
			+ "Lower values send metrics faster. Default: 60000")
	long metricExportInterval() default 60000;

	@AttributeDefinition(name = "Log Schedule Delay (ms)", description = "Delay between consecutive log record exports in milliseconds. Default: 5000")
	long logScheduleDelay() default 5000;

	@AttributeDefinition(name = "Log Max Export Batch Size", description = "Maximum number of log records exported per batch. Default: 512")
	int logMaxExportBatchSize() default 512;

	// --- Metrics ---

	@AttributeDefinition(name = "Aggregation Temporality", description = "Controls how metric data points are aggregated over time. "
			...
			+ "commercial backends (e.g. Datadog, Dynatrace).", options = {
					@Option(label = "Cumulative", value = "cumulative"), @Option(label = "Delta", value = "delta") })
	String aggregationTemporality() default "cumulative";

	// --- TLS ---

	@AttributeDefinition(name = "Trusted Certificates Path", description = "File path to a PEM-encoded certificate bundle containing trusted CA "
	...
	@AttributeDefinition(name = "Client Certificate Path", description = "File path to a PEM-encoded client certificate for mutual TLS (mTLS) "
	...
	@AttributeDefinition(name = "Client Key Path", description = "File path to a PEM-encoded client private key in PKCS#8 format for "
			+ "mutual TLS (mTLS) authentication. Must be used together with " + "'clientCertificatePath'.")
