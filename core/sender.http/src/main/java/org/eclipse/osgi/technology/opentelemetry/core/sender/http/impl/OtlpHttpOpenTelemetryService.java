 ...
 * <p>
 * This service has a higher service ranking than the logging exporter, ensuring
 * it is preferred by consumers when both are active.
 * <p>
 * The OTLP endpoint can be overridden via the
 * {@code OTEL_EXPORTER_OTLP_ENDPOINT} environment variable. Activates when a
 * configuration with PID {@value OtlpHttpOpenTelemetryConfiguration#PID}
 * exists.
 */
@Component(name = OtlpHttpOpenTelemetryConfiguration.COMPONENT_NAME, service = OpenTelemetry.class, configurationPid = OtlpHttpOpenTelemetryConfiguration.PID, configurationPolicy = ConfigurationPolicy.REQUIRE, scope = ServiceScope.SINGLETON)
@ServiceRanking(100)
@Designate(ocd = OtlpHttpOpenTelemetryConfiguration.class)
public class OtlpHttpOpenTelemetryService
		extends org.eclipse.osgi.technology.opentelemetry.core.commons.AbstractOpenTelemetryService {
	...
	private OpenTelemetrySdk buildSdk(BundleContext context, OtlpHttpOpenTelemetryConfiguration config) {
		...
		Resource resource = buildResource(context, resolveServiceName(config.serviceName()), config.serviceVersion(),
				config.serviceNamespace(), config.additionalResourceAttributes());

		// Span exporter
		OtlpHttpSpanExporterBuilder spanBuilder = OtlpHttpSpanExporter.builder().setEndpoint(endpoint + "/v1/traces")
				.setTimeout(timeout).setConnectTimeout(connectTimeout).setCompression(compression);
		...
		SdkTracerProvider tracerProvider = SdkTracerProvider.builder().setResource(resource)
				.addSpanProcessor(BatchSpanProcessor.builder(spanBuilder.build())
						.setScheduleDelay(toDuration(config.spanScheduleDelay()))
						.setMaxExportBatchSize(config.spanMaxExportBatchSize())
						.setMaxQueueSize(config.spanMaxQueueSize()).build())
				.build();

		// Metric exporter
		OtlpHttpMetricExporterBuilder metricBuilder = OtlpHttpMetricExporter.builder()
				.setEndpoint(endpoint + "/v1/metrics").setTimeout(timeout).setConnectTimeout(connectTimeout)
		...
		SdkMeterProvider meterProvider = SdkMeterProvider
				.builder().setResource(resource).registerMetricReader(PeriodicMetricReader
						.builder(metricBuilder.build()).setInterval(toDuration(config.metricExportInterval())).build())
				.build();

		// Log exporter
		OtlpHttpLogRecordExporterBuilder logBuilder = OtlpHttpLogRecordExporter.builder()
				.setEndpoint(endpoint + "/v1/logs").setTimeout(timeout).setConnectTimeout(connectTimeout)
				.setCompression(compression);
		headers.forEach(logBuilder::addHeader);
		configureTls(logBuilder, trustedCerts, clientCert, clientKey);

		SdkLoggerProvider loggerProvider = SdkLoggerProvider.builder().setResource(resource)
				.addLogRecordProcessor(BatchLogRecordProcessor.builder(logBuilder.build())
						.setScheduleDelay(toDuration(config.logScheduleDelay()))
						.setMaxExportBatchSize(config.logMaxExportBatchSize()).build())
				.build();
	...
	private void configureTls(OtlpHttpMetricExporterBuilder builder, byte[] trustedCerts, byte[] clientCert,
			byte[] clientKey) {
		if (trustedCerts != null) {
			builder.setTrustedCertificates(trustedCerts);
		}
	...
	private void configureTls(OtlpHttpLogRecordExporterBuilder builder, byte[] trustedCerts, byte[] clientCert,
			byte[] clientKey) {
		if (trustedCerts != null) {
			builder.setTrustedCertificates(trustedCerts);
		}
