package org.eclipse.osgi.technology.opentelemetry.core.sender.http.impl.integration;
...
import org.eclipse.osgi.technology.opentelemetry.core.sender.http.api.Constants;
...
public class HttpSenderIntegrationTest {
	...
	@Test
	@WithConfiguration(pid = Constants.PID, properties = { @Property(key = "serviceName", value = "test-http-sender"),
			@Property(key = "endpoint", value = "http://localhost:18319") })
	void serviceIsRegisteredAfterConfiguration(@InjectService(timeout = 3000) OpenTelemetry openTelemetry) {
		assertThat(openTelemetry).isNotNull();
	}

	@Test
	@WithConfiguration(pid = Constants.PID, properties = { @Property(key = "serviceName", value = "test-http-traces"),
			@Property(key = "endpoint", value = "http://localhost:18319") })
	void sendsTracesToEndpoint(@InjectService(timeout = 3000) OpenTelemetry openTelemetry) throws Exception {
		receivedPaths.clear();
		traceLatch = new CountDownLatch(1);
		...
		assertThat(received).as("Expected trace data at /v1/traces within 15s").isTrue();
		assertThat(receivedPaths).anyMatch(p -> p.contains("/v1/traces"));
	}

	@Test
	@WithConfiguration(pid = Constants.PID, properties = {
			@Property(key = "serviceName", value = "test-http-content-type"),
			@Property(key = "endpoint", value = "http://localhost:18319") })
	void sendsProtobufContentType(@InjectService(timeout = 3000) OpenTelemetry openTelemetry) throws Exception {
	...
	void pidConstantIsCorrect() {
		assertThat(Constants.PID).isEqualTo("org.eclipse.osgi.technology.opentelemetry.core.sender.http");
	}
}
