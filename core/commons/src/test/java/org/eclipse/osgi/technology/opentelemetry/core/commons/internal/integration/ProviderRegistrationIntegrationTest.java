package org.eclipse.osgi.technology.opentelemetry.core.commons.internal.integration;
...
public class ProviderRegistrationIntegrationTest {
	...
	void providersAppearWhenOpenTelemetryIsRegistered(
			...
			@InjectService(cardinality = 0) ServiceAware<ContextPropagators> propagatorsAware) throws Exception {

		assertThat(tracerAware.size()).isZero();

		var props = new Hashtable<String, Object>();
		props.put("component.name", "test-otel");
		ServiceRegistration<OpenTelemetry> reg = bc.registerService(OpenTelemetry.class, createDistinctNoop(), props);
		...
		assertThat(tracerAware.getServiceReference().getProperty("opentelemetry.name")).isEqualTo("test-otel");
	...
	void multipleOpenTelemetryInstancesCreateMultipleProviders(
			@InjectService(cardinality = 0) ServiceAware<TracerProvider> tracerAware) throws Exception {

		var props1 = new Hashtable<String, Object>();
		props1.put("component.name", "otel-one");
		ServiceRegistration<OpenTelemetry> reg1 = bc.registerService(OpenTelemetry.class, createDistinctNoop(), props1);
		...
		ServiceRegistration<OpenTelemetry> reg2 = bc.registerService(OpenTelemetry.class, createDistinctNoop(), props2);
