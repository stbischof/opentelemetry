package org.eclipse.osgi.technology.opentelemetry.integration.framework.integration;
...
@WithConfiguration(
    ...
    properties = {
        @Property(key = "serviceName", value = "test-framework")
    }
)
public class FrameworkIntegrationTest {
    ...
    void canCreateTracer(
            @InjectService(timeout = 3000) OpenTelemetry openTelemetry) {
        var tracer = openTelemetry.getTracer("integration-test-framework");
    ...
    void canCreateMeter(
            @InjectService(timeout = 3000) OpenTelemetry openTelemetry) {
        var meter = openTelemetry.getMeter("integration-test-framework");
