...
public class ConfigAdminEventComponent implements ConfigurationListener {
	...
	public void configurationEvent(ConfigurationEvent event) {
		try {
			...
			Span span = tracer.spanBuilder(spanName).setSpanKind(SpanKind.INTERNAL)
					.setAttribute(AttributeKey.stringKey("cm.pid"), event.getPid())
					.setAttribute(AttributeKey.stringKey("cm.event.type"), eventType).startSpan();
			...
			try (Scope ignored = span.makeCurrent()) {
				...
				if (event.getReference() != null) {
					span.setAttribute("cm.service.bundle", event.getReference().getBundle().getSymbolicName());
				}
