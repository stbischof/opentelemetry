package org.eclipse.osgi.technology.opentelemetry.integration.cm.integration;
...
@WithConfiguration(
    ...
    properties = {
        @Property(key = "serviceName", value = "test-cm")
    }
)
public class CmIntegrationTest {
    ...
    void canCreateTracer(
            @InjectService(timeout = 3000) OpenTelemetry openTelemetry) {
        var tracer = openTelemetry.getTracer("integration-test-cm");
    ...
    void canCreateMeter(
            @InjectService(timeout = 3000) OpenTelemetry openTelemetry) {
        var meter = openTelemetry.getMeter("integration-test-cm");
