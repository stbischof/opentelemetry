...
/**
 * Active telemetry producer that creates custom business metrics using
 ...
 * telemetry because:
 * <ul>
 *   <li>Telemetry can be expensive — the service may be deactivated at runtime</li>
 ...
 *   <li>When OpenTelemetry arrives, instruments are created; when it leaves,
 *       the component falls back to no-ops</li>
 * </ul>
 */
...
public class MetricsProducerService {
    ...
    void bindOpenTelemetry(OpenTelemetry openTelemetry) {
        ...
        activeTasksCounter = meter.upDownCounterBuilder("example.tasks.active")
                .setDescription("Currently active tasks")
                .setUnit("{tasks}")
                .build();

        storeGauge = meter.gaugeBuilder("example.store.size")
                .setDescription("Number of items in the store")
                .setUnit("{items}")
                .ofLongs()
                .buildWithCallback(m -> m.record(storeSize.get()));
    }
    ...
    public void recordRequest(String type, String path) {
        ...
        if (c != null) {
            c.add(1, Attributes.of(
                    AttributeKey.stringKey("request.type"), type,
    ...
    public void recordDuration(double millis) {
        ...
        if (h != null) {
            h.record(millis, Attributes.of(
                    AttributeKey.stringKey("operation"), "demo"));
        }
    }
