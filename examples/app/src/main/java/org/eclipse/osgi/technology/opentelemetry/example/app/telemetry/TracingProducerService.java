 ...
 * bind/unbind. When OpenTelemetry is not available, all tracing operations
 * are silently skipped. This allows the component to remain active even
 * when telemetry is disabled or the service is temporarily unavailable.
 */
...
public class TracingProducerService {
    ...
    public void executeWithTracing(String operationName, Runnable work) {
        ...
        Span span = t.spanBuilder(operationName)
                .setSpanKind(SpanKind.INTERNAL)
                .setAttribute("example.operation", operationName)
                .startSpan();
    ...
    public void executeNestedOperation() {
        ...
        Span parentSpan = t.spanBuilder("example.parent-operation")
                .setSpanKind(SpanKind.SERVER)
                .startSpan();
        try (Scope parentScope = parentSpan.makeCurrent()) {
            parentSpan.addEvent("Parent started");

            Span childSpan = t.spanBuilder("example.child-operation")
                    .setSpanKind(SpanKind.CLIENT)
                    .setAttribute("example.child.step", "lookup")
                    .startSpan();
            try (Scope childScope = childSpan.makeCurrent()) {
                childSpan.addEvent("Child processing",
                        Attributes.of(AttributeKey.stringKey("detail"), "step-1"));
                simulateWork(50);
