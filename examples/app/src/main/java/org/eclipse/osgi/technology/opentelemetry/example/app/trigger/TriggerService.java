...
public class TriggerService {
	...
	public String triggerJaxRs() {
		...
		callPostEndpoint("http://localhost:8181/rest1/orders", "{\"name\":\"trigger-item\",\"data\":\"from-trigger\"}",
				report);
		callEndpoint("http://localhost:8181/rest1/orders", report);
		callEndpoint("http://localhost:8181/rest1/orders/1", report);
		return report.toString();
	}
	...
	public String triggerTelemetry() {
		...
		if (lp != null) {
			lp.logBusinessEvent("trigger.executed",
					Map.of("source", "TriggerService", "timestamp", Instant.now().toString()));
	...
	public String triggerJdbc() {
		...
		try {
			long id = jdbc.insertEvent("trigger", "Triggered at " + Instant.now());
	...
	private void callEndpoint(String url, StringBuilder report) {
		try {
			HttpRequest req = HttpRequest.newBuilder().uri(URI.create(url)).timeout(Duration.ofSeconds(5)).GET()
					.build();
			HttpResponse<String> resp = httpClient.send(req, HttpResponse.BodyHandlers.ofString());
			report.append("GET ").append(url).append(" -> ").append(resp.statusCode()).append("\n");
		} catch (Exception e) {
			report.append("GET ").append(url).append(" -> FAILED: ").append(e.getMessage()).append("\n");
		}
	}

	private void callPostEndpoint(String url, String body, StringBuilder report) {
		try {
			HttpRequest req = HttpRequest.newBuilder().uri(URI.create(url)).timeout(Duration.ofSeconds(5))
					.header("Content-Type", "application/json").POST(HttpRequest.BodyPublishers.ofString(body)).build();
			HttpResponse<String> resp = httpClient.send(req, HttpResponse.BodyHandlers.ofString());
			report.append("POST ").append(url).append(" -> ").append(resp.statusCode()).append("\n");
		} catch (Exception e) {
			report.append("POST ").append(url).append(" -> FAILED: ").append(e.getMessage()).append("\n");
		}
	}
}
