...
public class ScrWeaverIntegrationTest {
	...
	void componentActivationCreatesSpan() throws Exception {
		...
		Bundle testAppBundle = findBundle("org.eclipse.osgi-technology.opentelemetry.weaving.osgi.scr.test.app");
		...
		SpanData span = exportedSpans.stream().filter(s -> s.getName().contains("scr.activate"))
				.filter(s -> s.getName().contains("TestComponent")).findFirst().orElseThrow(() -> new AssertionError(
						"No scr.activate span for TestComponent found. Spans: " + exportedSpans));

		assertThat(span.getKind()).isEqualTo(SpanKind.INTERNAL);
		assertThat(span.getAttributes().get(AttributeKey.stringKey("scr.lifecycle.action"))).isEqualTo("activate");
		assertThat(span.getAttributes().get(AttributeKey.stringKey("scr.component.class"))).contains("TestComponent");
	}
