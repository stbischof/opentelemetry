...
public class JdbcWeaverIntegrationTest {
    ...
    void selectQueryCreatesSpan() throws Exception {
        ...
        SpanData span = exportedSpans.stream()
                .filter(s -> s.getName().equals("SELECT"))
                .findFirst()
                .orElseThrow(() -> new AssertionError(
                        "No SELECT span found. Spans: " + exportedSpans));

        assertThat(span.getKind()).isEqualTo(SpanKind.CLIENT);
        assertThat(span.getAttributes().get(
                AttributeKey.stringKey("db.system"))).isEqualTo("jdbc");
        assertThat(span.getAttributes().get(
                AttributeKey.stringKey("db.statement"))).contains("SELECT");
        assertThat(span.getAttributes().get(
                AttributeKey.stringKey("db.operation"))).isEqualTo("executeQuery");
    }
    ...
    void insertCreatesSpan() throws Exception {
        ...
        SpanData span = exportedSpans.stream()
                .filter(s -> s.getName().equals("INSERT"))
                .findFirst()
                .orElseThrow(() -> new AssertionError(
                        "No INSERT span found. Spans: " + exportedSpans));

        assertThat(span.getKind()).isEqualTo(SpanKind.CLIENT);
        assertThat(span.getAttributes().get(
                AttributeKey.stringKey("db.system"))).isEqualTo("jdbc");
        assertThat(span.getAttributes().get(
                AttributeKey.stringKey("db.statement"))).contains("INSERT");
    }
}
